...
class Register:
    ...
    def load(self, value: int):
        ...
        # Преобразуем число в 32-битный список битов
        bits = [int(x) for x in bin(value)[2:]]  # Убираем '0b' в начале
        # Дополняем нулями слева до 32 бит
    ...
    def add(self, other: 'Register') -> 'Register':
        ...
        # Обрезаем до 32 бит, если было переполнение
        if overflow_occurred:
            sum_val = sum_val & 0xFFFFFFFF  # Эквивалентно обрезанию до 32 бит
    ...
    def mul(self, other: 'Register') -> 'Register':
        ...
        if overflow_occurred:
            mul_val = mul_val & 0xFFFFFFFF  # Обрезаем до 32 бит
